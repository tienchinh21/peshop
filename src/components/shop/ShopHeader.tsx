...
export function ShopHeader({
  ...
  const generateBreadcrumbs = () => {
    ...
    if (!route) {
      ...
      } else if (pathname.match(/^\/shop\/chien-dich\/ma-giam-gia\/[^\/]+$/) && !pathname.match(/^\/shop\/chien-dich\/ma-giam-gia\/(them|sua)$/)) {
        route = routeMap["/shop/chien-dich/ma-giam-gia/[id]"];
      } else if (pathname.match(/^\/shop\/san-pham\/[^\/]+$/) && !pathname.match(/^\/shop\/san-pham\/(them)$/)) {
  ...
  return <header className="sticky top-0 z-50 w-full border-b bg-white shadow-sm">
      <div className="flex h-14 items-center justify-between px-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          {}
          {breadcrumbs.length > 0 && <div className="hidden md:block h-4 w-px bg-gray-300"></div>}

          {}
          {breadcrumbs.length > 0 && <div className="hidden md:block">
              <Breadcrumb>
                <BreadcrumbList>
                  {breadcrumbs.map((breadcrumb, index) => <React.Fragment key={index}>
                      <BreadcrumbItem>
                        {index === breadcrumbs.length - 1 ? <BreadcrumbPage className="text-sm font-medium text-gray-900">
                            {breadcrumb.title}
                          </BreadcrumbPage> : <BreadcrumbLink asChild className="text-sm text-gray-600 hover:text-gray-900">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              <DropdownMenuItem asChild>
                <Link href="/shop/cai-dat" className="flex items-center space-x-2 cursor-pointer">
                  <Settings className="h-4 w-4" />
                  <span>Cài đặt</span>
                </Link>
              </DropdownMenuItem>
              <DropdownMenuItem asChild>
                <Link href="/tai-khoan" className="flex items-center space-x-2 cursor-pointer">
