 ...
 * Sidebar với menu 2 cấp cho quản lý shop
 */
export function ShopSidebar() {
  const pathname = usePathname();
  const [openMenus, setOpenMenus] = React.useState<string[]>([]);

  // Auto-open menu nếu submenu đang active
  React.useEffect(() => {
                  ...
                  {item.items ? (
                    ...
                    <>
                      <SidebarMenuButton
                        ...
                        isActive={openMenus.includes(item.title)}
                        className="cursor-pointer px-4 py-3 h-auto min-h-[48px] text-sm font-medium"
                      >
                          ...
                          {item.items.map((subItem) => (
                            <SidebarMenuSubItem key={subItem.title}>
                              <SidebarMenuSubButton
                                ...
                                className="px-4 py-2 h-auto min-h-[40px] text-sm"
                              >
                    ...
                    <SidebarMenuButton
                      ...
                      className="px-4 py-3 h-auto min-h-[48px] text-sm font-medium"
                    >
                      <Link href={item.url}>
                        <item.icon className="w-5 h-5" />
                        <span className="text-sm">{item.title}</span>
