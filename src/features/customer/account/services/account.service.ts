...
export const updateProfile = async (
  ...
  try {
    const response = await axiosDotnet.put<UpdateProfileResponse>(
      API_ENDPOINTS.AUTH.ME,
      data
    );
...
export const changePassword = async (
  ...
  try {
    const response = await axiosDotnet.post<ChangePasswordResponse>(
      `${API_ENDPOINTS.AUTH.ME}/change-password`,
      data
    );
    return response.data;
  } catch (error: unknown) {
