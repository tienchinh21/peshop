...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = error?.response?.data?.error || error?.response?.data?.message || "Đăng nhập thất bại. Vui lòng thử lại.";
      toast.error(errorMessage);
      setGeneralError(errorMessage);
    } finally {
      setIsLoading(false);
  ...
  return <div className="h-screen w-full  flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden lg:block">
            <div className="relative">
              ...
              <div className="relative bg-white rounded-3xl p-8 shadow-2xl">
                <Image src="/01.png" alt="WebCore Incom - Hệ thống quản lý" width={500} height={400} className="w-full h-auto rounded-2xl" priority />
              ...
              <Card className="shadow-xl border-0 bg-white/80 backdrop-blur-sm py-6">
                ...
                <CardContent>
                  <form onSubmit={handleSubmit} className="space-y-6">
                    {}
                    {generalError && <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                        <p className="text-sm text-red-600">{generalError}</p>
                      </div>}

                    {}
                    <FormField id="email" name="email" type="email" label="Email" placeholder="Nhập email của bạn" value={formData.email} onChange={handleInputChange} error={errors.email} icon={<Mail className="h-4 w-4" />} required pattern={VALIDATION_PATTERNS.email} />

                    {}
                    <PasswordField id="password" name="password" label="Mật khẩu" placeholder="Nhập mật khẩu của bạn" value={formData.password} onChange={handleInputChange} error={errors.password} required />
                    ...
                    {}
                    <Button type="submit" disabled={isLoading} className="w-full bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 text-white font-medium py-2.5 rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed">
                  ...
                  <div className="mt-6 text-center">
                    <p className="text-sm text-gray-600">
                      Chưa có tài khoản?{" "}
                      <button type="button" onClick={() => router.push("/dang-ky")} className="text-primary hover:text-primary/80 font-medium cursor-pointer">
                        Đăng ký ngay
                      </button>
                    </p>
                  </div>
