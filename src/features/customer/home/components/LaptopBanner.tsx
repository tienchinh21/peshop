        ...
        <CarouselContent className="h-full">
          {slides.map((slide) => (
            <CarouselItem key={slide.id} className="h-full">
              <div
                className="p-3 md:p-6 relative overflow-hidden h-full w-full"
              ...
              >
                ...
                <div className="relative z-10 max-w-xs h-full">
                  <p
                    className={`text-xs font-bold mb-1 md:mb-2 tracking-wider uppercase ${
                      slide.beforeTitleColor || "text-white"
                    }`}
                  >
                    {slide.beforeTitle}
                  </p>
                  <h2
                    className={`text-lg md:text-2xl font-bold mb-2 md:mb-3 leading-tight whitespace-pre-line ${
                  ...
                  <p
                    className={`mb-2 md:mb-4 text-xs md:text-sm ${
                      slide.descriptionColor || "text-white"
                    }`}
                  >
                    {slide.description}
                  </p>
                  <Button
                    // variant={slide.buttonVariant || "secondary"}
                    variant="secondary"
                    size="sm"
                    className={
                      slide.buttonClassName ||
                      "bg-white hover:bg-gray-100 text-gray-800 rounded-md text-xs md:text-sm mt-3 md:mt-5"
                    }
                  >
                    {slide.buttonText}
                  </Button>
