...
import { createCachedFetcher, CACHE_REVALIDATION } from "@/shared/services/cache";
import type { ProductsApiResponse, ProductFilters, ProductDetail, ProductDetailApiResponse } from "../types";
export const getProductsServer = async (filters?: ProductFilters): Promise<ProductsApiResponse> => {
  ...
  const endpoint = queryString ? `${API_ENDPOINTS.PRODUCTS.GET_PRODUCTS}?${queryString}` : API_ENDPOINTS.PRODUCTS.GET_PRODUCTS;
...
export const getProductsServerCached = createCachedFetcher(async (page?: number, pageSize?: number, categoryId?: string, categoryChildId?: string, minPrice?: number, maxPrice?: number, reviewPoint?: number): Promise<ProductsApiResponse> => {
  const baseUrl = API_CONFIG.BASE_URL;
  if (!baseUrl) {
    throw new Error("NEXT_PUBLIC_API_URL_DOTNET is not configured");
  }
...
export const getTopProductSlugs = createCachedFetcher(async (limit: number = 100): Promise<string[]> => {
  ...
  try {
    ...
    return products.filter((product: any) => product.slug).map((product: any) => product.slug);
  } catch (error) {
