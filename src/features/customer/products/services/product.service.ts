...
import type { ProductsApiResponse, ProductFilters, Product, ProductDetail, ProductDetailApiResponse, ProductPromotion, ProductPromotionsResponse, OrderPromotion } from "../types";
export const getProducts = async (filters?: ProductFilters): Promise<ProductsApiResponse> => {
  ...
  const url = queryString ? `${API_ENDPOINTS.PRODUCTS.GET_PRODUCTS}?${queryString}` : API_ENDPOINTS.PRODUCTS.GET_PRODUCTS;
  const response = await axiosDotnet.get<ProductsApiResponse>(url);
  return response.data;
};
export const searchProducts = async (filters?: ProductFilters): Promise<ProductsApiResponse> => {
  const params = new URLSearchParams();
  if (filters?.keyword) params.append("keyword", filters.keyword);
  if (filters?.page) params.append("page", filters.page.toString());
  if (filters?.pageSize) params.append("pageSize", filters.pageSize.toString());
  const queryString = params.toString();
  const url = queryString ? `${API_ENDPOINTS.SEARCH.SEARCH}?${queryString}` : API_ENDPOINTS.SEARCH.SEARCH;
  const response = await axiosDotnet.get<ProductsApiResponse>(url);
  return response.data;
};
...
export const getSimilarProducts = async (productId: string, options?: {
  byCategory?: boolean;
  byShop?: boolean;
  limit?: number;
}): Promise<Product[]> => {
...
export const getPromotionsByProduct = async (productId: string): Promise<ProductPromotion[]> => {
  const params = new URLSearchParams();
  params.append("productId", productId);
...
export const checkPromotionsInOrder = async (orderId: string): Promise<OrderPromotion[]> => {
  const params = new URLSearchParams();
...
export const viewProduct = async (productId: string): Promise<void> => {
  await axiosDotnet.post(API_ENDPOINTS.USER.VIEW_PRODUCT, { productId });
};