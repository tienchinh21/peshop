...
export default function SearchResultsPage() {
  ...
  const [imageSearchData, setImageSearchData] =
    useState<ImageSearchResultData | null>(null);
  useEffect(() => {
    if (isImageSearch) {
  ...
  const getDescription = () => {
    if (isImageSearch) {
      return totalCount > 0
        ? `Tìm thấy ${totalCount} sản phẩm tương tự với hình ảnh`
  ...
  return (
    <div className="min-h-screen">
      <PageSection
        title={isImageSearch ? "Tìm kiếm bằng hình ảnh" : "Kết quả tìm kiếm"}
      ...
      <SectionContainer>
        <div className="py-8">
          ...
          {}
          {!isPageLoading && !error && !keyword && !isImageSearch && (
          ...
          {!isPageLoading &&
            ...
            products.length === 0 && (
              <div className="bg-white rounded-lg shadow-sm p-12 text-center">
                ...
                <p className="text-gray-500">
                  {isImageSearch
                    ? "Thử tải lên hình ảnh khác để tìm kiếm sản phẩm tương tự"
                    : "Thử thay đổi từ khóa để tìm kiếm sản phẩm khác"}
                </p>
              </div>
            )}
