...
import { useShopSendMessage } from "../hooks/useShopSendMessage";
import { useShopChatRealtime } from "../hooks/useShopChatRealtime";
import { useShopMarkAsRead } from "../hooks/useShopMarkAsRead";
import { useShopChatContextSafe } from "../context";
import type { Conversation } from "@/features/customer/chat/types/chat.types";

function getPartnerInfo(conversation: Conversation): {
  name: string;
  avatar: string;
} {
  return {
...
export function ShopChatWidget() {
  const chatContext = useShopChatContextSafe();
  ...
  const { messages, isLoading: isLoadingMessages } = useShopMessages(
    selectedConversation?.userId ?? "",
    selectedConversation?.shopId ?? ""
  );

  const { markAsRead } = useShopMarkAsRead();

  const { sendMessage, isPending: isSending } = useShopSendMessage({
    onSuccess: () => {
      setMessageInput("");
  ...
  const handleSelectConversation = useCallback(
    (conv: Conversation | null) => {
      ...
      if (conv) markAsRead(conv.userId, conv.shopId);
    },
    [markAsRead]
  );
  ...
  const partnerInfo = selectedConversation
    ? getPartnerInfo(selectedConversation)
    : null;
