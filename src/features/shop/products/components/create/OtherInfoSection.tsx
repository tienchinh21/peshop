...
export function OtherInfoSection({
  ...
  const { data: templateResponse, isLoading: loading } = useCategoryTemplate(
  ...
  React.useEffect(() => {
    if (categoryTemplate) {
      ...
      categoryTemplate.templateCategories.forEach(
      ...
      categoryTemplate.templateCategoryChildren.forEach(
        (template: TemplateCategoryChild) => {
          initialInfos.push({
            name: template.name,
            value: "",
          });
      ...
      {loading ? (
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            ].map((template: TemplateCategory | TemplateCategoryChild) => (
              <AttributeField
...
function AttributeField({
  ...
  const suggestedOptions =
    template.attributeTemplates?.map((attr: { name: string }) => attr.name) ||
    [];
