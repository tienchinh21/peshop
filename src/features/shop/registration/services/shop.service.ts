...
import type { Shop, CreateShopRequest } from "../types";
export const createShop = async (data: CreateShopRequest) => {
  const shopData = {
    ...
    ...(data.newProviceId && {
      newProviceId: data.newProviceId
    }),
    ...(data.newWardId && {
      newWardId: data.newWardId
    }),
    ...(data.fullNewAddress && {
      fullNewAddress: data.fullNewAddress
    })
  };
  const formData = createFormDataWithJson(shopData, data.logofile);
  const response = await axiosJavaFormData(API_ENDPOINTS_JAVA.SHOPS.CREATE, formData, "POST");
  return response;
};
export const updateShop = async (id: string, data: CreateShopRequest) => {
  ...
  const formData = createFormDataWithJson(shopData, data.logofile);
  const response = await axiosJavaFormData(API_ENDPOINTS_JAVA.SHOPS.PUT.replace(":id", id), formData, "PUT");
  return response;
};
export const getShop = async (id: string) => {
  const response = await axiosJava.get<Shop>(API_ENDPOINTS_JAVA.SHOPS.PUT.replace(":id", id));
