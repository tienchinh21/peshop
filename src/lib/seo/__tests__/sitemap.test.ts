import { generateStaticSitemap, generateProductSitemap, generateShopSitemap, generateCompleteSitemap } from '../sitemap.service';
...
generateProductSitemap().then(productEntries => {
  ...
  const priorityOrderCorrect = priority.homepage === 1.0 && priority.categories === 0.8 && priority.products === 0.6 && priority.shops === 0.6 && priority.static === 0.4 && priority.homepage > priority.categories && priority.categories > priority.products && priority.products >= priority.shops && priority.shops > priority.static;
  console.log('✓ Priority ordering is correct:', priorityOrderCorrect);
  const {
    changeFrequency
  } = seoConfig.sitemap;
  const changeFreqValid = validFrequencies.includes(changeFrequency.homepage) && validFrequencies.includes(changeFrequency.products) && validFrequencies.includes(changeFrequency.shops) && validFrequencies.includes(changeFrequency.categories) && validFrequencies.includes(changeFrequency.static);
