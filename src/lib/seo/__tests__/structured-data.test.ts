/**
 * Manual verification tests for structured data generation
 * Run with: npx tsx src/lib/seo/__tests__/structured-data.test.ts
 */
...
const productSchema = generateProductSchema(
  mockProduct,
  'https://peshop.vn/san-pham/iphone-15-pro-max-256gb'
);

console.log('Product Schema:', JSON.stringify(productSchema, null, 2));
console.log('\n');

// Test Product Schema with single variant
...
const singleVariantSchema = generateProductSchema(
...
// Test Product Schema without reviews
...
const noReviewSchema = generateProductSchema(
  noReviewProduct,
  'https://peshop.vn/san-pham/iphone-15-pro-max-256gb'
);
...
const shopSchema = generateShopSchema(
  mockShop,
  'https://peshop.vn/shop-view/apple-store-vietnam'
);
...
const breadcrumbSchema = generateBreadcrumbSchema([
  ...
  { name: 'Điện thoại', url: '/san-pham?category=dien-thoai' },
  { name: 'iPhone 15 Pro Max' }, // Last item without URL (current page)
]);
...
// Check 1: Product schema has required fields
...
// Check 2: Product schema with reviews has aggregateRating
...
// Check 3: Multi-variant product uses AggregateOffer
...
// Check 4: Shop schema has required fields
...
// Check 5: Breadcrumb schema has correct structure
const hasBreadcrumbStructure = (schema: any) => {
  return (
    ...
    schema.itemListElement.every((item: any, index: number) => 
      item['@type'] === 'ListItem' &&
...
// Check 6: Last breadcrumb item doesn't have URL
...
// Check 7: Organization schema has required fields
...
// Check 8: WebSite schema has search action
...
// Check 9: Availability is correct
const hasStockProduct = productSchema.offers;
console.log('✓ Product with stock has InStock availability:', 
  hasStockProduct.availability === 'https://schema.org/InStock'
);

// Check 10: Out of stock product
...
console.log('✓ Product without stock has OutOfStock availability:', 
  outOfStockSchema.offers.availability === 'https://schema.org/OutOfStock'
);

// Check 11: Serialized schema is valid JSON
...
// Check 12: AggregateRating values are clamped correctly
const clampedRating = generateAggregateRating(6.5, 0); // Should clamp to 5 and 1
