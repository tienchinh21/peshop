...
export const getVoucherById = async (
  id: string
): Promise<ShopVoucherDetailResponse> => {
  const response = await axiosJava.get<ShopVoucherDetailResponse>(
    `/voucher-shop/${id}`
  );
  return response.data;
};
...
export const updateVoucher = async (
  ...
  const response = await axiosJava.put<ShopVoucherDetailResponse>(
    `/voucher-shop/${id}`,
    payload
  );
  return response.data;
