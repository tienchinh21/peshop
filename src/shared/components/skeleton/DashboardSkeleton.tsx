...
export function TodoListSkeleton() {
  return <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[1, 2, 3, 4].map(i => <div key={i} className="border-2 rounded-lg p-4 border-gray-200">
...
export function DashboardChartsSkeleton() {
  return <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
      {}
      {[1, 2, 3, 4].map(i => <Card key={i} className="lg:col-span-2">
          <CardHeader>
          ...
          <CardContent>
            <div className="space-y-3">
              {}
              {[1, 2, 3, 4, 5].map(j => <div key={j} className="flex items-center gap-3">
                  <Skeleton className="h-3 w-16" />
                  <Skeleton className="h-8 flex-1" style={{
              maxWidth: `${Math.random() * 60 + 40}%`
            }} />
                </div>)}
            </div>
          </CardContent>
