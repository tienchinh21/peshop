...
export interface UseFormValidationReturn {
  ...
  validateAll: (fields: Record<string, {
    value: string;
...
const getErrorMessage = (fieldName: string, rule: keyof ValidationRule, ruleValue?: number | RegExp): string => {
...
export const useFormValidation = (): UseFormValidationReturn => {
  ...
  const validate = useCallback((name: string, value: string, rules: ValidationRule): boolean => {
  ...
  const validateAll = useCallback((fields: Record<string, {
    value: string;
    rules: ValidationRule;
  }>): boolean => {
    let isValid = true;
