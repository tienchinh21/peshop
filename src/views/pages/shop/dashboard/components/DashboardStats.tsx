...
import { DashboardData, DashboardMetric } from "@/types/shops/dashboard.type";
import {
  ArrowDownIcon,
  ArrowUpIcon,
  DollarSign,
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      ...
      <StatCard
        ...
        metric={data.orderConversionRate}
        icon={<Percent />}
        isPercentage
      />
      <StatCard
        title="Lượt click sản phẩm"
        metric={data.productClicks}
        icon={<MousePointer />}
      />
      <StatCard
        title="Đơn hàng bị hủy"
        metric={data.ordersCancelled}
        icon={<AlertCircle />}
      />
      <StatCard
        title="Doanh thu/Đơn"
        metric={data.revenuePerOrder}
        icon={<DollarSign />}
        isCurrency
      />
      <StatCard
        title="Doanh thu hủy"
        metric={data.cancellationSales}
        icon={<DollarSign />}
        isCurrency
      />
    </div>
