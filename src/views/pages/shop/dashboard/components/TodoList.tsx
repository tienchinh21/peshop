...
import { TodoListContent } from "@/types/shops/dashboard.type";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {todoItems.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className={`relative border-2 rounded-lg p-4 transition-all duration-200 hover:shadow-md cursor-pointer ${item.color}`}
            >
              {/* Badge with count */}
              {item.count > 0 && (
                <div className="absolute -top-2 -right-2">
                  <div
                    className={`${item.badge} text-white text-xs font-bold rounded-full h-6 w-6 flex items-center justify-center shadow-md`}
                  >
      ...
      <div className="mt-4 pt-4 border-t border-gray-200">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="font-bold text-gray-900">
            {data.waitingForDelivery +
              data.processed +
              data.returnsAndCancellations +
              data.productlocked}{" "}
            việc
          </span>
        </div>
