...
const EmailVerificationStep = lazy(
  () =>
    import("@/components/auth/EmailVerificationStep").then((module) => ({
      default: module.EmailVerificationStep,
    }))
);

const RegistrationFormStep = lazy(
  () =>
    import("@/components/auth/RegistrationFormStep").then((module) => ({
...
const RegisterPage: React.FC = () => {
  ...
  const { errors, validate, validateAll, clearError, clearAllErrors } = useFormValidation();
  ...
  const handleSendOtp = async () => {
    ...
    dispatch({ type: "SET_SENDING_OTP", sending: true });
    try {
      const response = await sendOtp({ email: state.formData.email });
      if (!response.error) {
        dispatch({ type: "SET_OTP_SENT", sent: true });
        startCountdown(60);
      ...
      dispatch({ type: "SET_SENDING_OTP", sending: false });
    }
  };
  ...
  const handleResendOtp = async () => {
    ...
    dispatch({ type: "SET_SENDING_OTP", sending: true });
    try {
      const response = await resendOtp({ email: state.formData.email });
      if (!response.error) {
        startCountdown(60);
  ...
  const handleVerifyOtp = async () => {
    ...
    dispatch({ type: "SET_VERIFYING_OTP", verifying: true });
    try {
      ...
      if (!response.error && response.data) {
        if (response.data.status) {
          dispatch({ type: "SET_VERIFICATION_KEY", key: response.data.key });
      ...
      dispatch({ type: "SET_VERIFYING_OTP", verifying: false });
    }
  };
                  ...
                  <div className="flex items-center justify-center gap-4">
                    ...
                    <div
                      className={`flex-1 h-1 rounded-full transition-all ${
                        state.currentStep > 1 ? "bg-green-500" : "bg-gray-200"
                      }`}
                      style={{ maxWidth: "60px" }}
                    />
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-sm font-medium hidden sm:block ${
                          state.currentStep === 2 ? "text-primary" : "text-gray-500"
                        }`}
                      >
                        Thông tin cá nhân
                      </span>
                ...
                >
                  {state.currentStep === 1 ? (
                    <EmailVerificationStep
                      ...
                      onOtpChange={(value) => handleFieldChange("otp", value)}
